...
export default function Home({ settingsJson, subscribesJson, usersJson }) {
  ...
  const deleteSubscribe = async (data) => {
    try {
      const response = await request(`http://localhost:4200/subscribes/${data.form.id}`, 'DELETE');
  ...
  const editSubscribe = async (data) => {
    try {
      await request(`http://localhost:4200/subscribes/${data.form.id}`, 'PUT', data.form);
    } catch(e) {
      console.log('Error', e)
    }
  }
    ...
    <MainLayout name={pageData.pageTitle}>
      <div className="box">
        <div className='box-left'>
          <GeneralSettings settings={settings} onEdit={udpdateSettings}></GeneralSettings>
        </div>
        <div className='box-right'>
          <GlobalSettings settings={settings} ></GlobalSettings>
          <SubscribesSettings onDelete={deleteSubscribe} onEdit={editSubscribe} onChange={addSubscribe} subscribe={subscribesJson}></SubscribesSettings>
          <CrmRolesSettings onAdd={(form) => addRole(form)} users={usersJson}></CrmRolesSettings>
        </div>
      </div>
    </MainLayout>
  )
