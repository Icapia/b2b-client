...
import { ChargePointStatus, ChargePointT, ConnectorStatus } from "@/types/site-types";
...
export const SiteGridRow: FC<SiteGridRowI> = ({
  ...
  const status: ChargePointStatus = chargepoint?.status?.toLowerCase() as ChargePointStatus || "disconnected"

  const handleNavigate = () => {
    router.push(`/charging-sites/${chargepoint.siteId}`)
  }

  const connectors = chargepoint?.connectors?.map((e) => `${e?.connectorTypeName}, `) || 'No connectors'
  ...
  return (
    <>
      <tr style={{overflowX: 'scroll'}}>
        ...
        <th><Typography fontSize={15} fontWeight={500}>{"Chargepoint #" + chargepoint?.id}</Typography></th>
        <td><Typography fontSize={15} fontWeight={500}>{address}</Typography></td>
        <td><Typography fontSize={15} fontWeight={500}>{site_name}</Typography></td>
        <td>
      ...
      <tr style={{overflowX: 'scroll'}}>
        <td style={{ height: 0, padding: 0 }} colSpan={8}>
          {(isOpen && chargepoint?.connectors && chargepoint.connectors?.length > 0) && (
            <Sheet
              variant="soft"
              sx={{
                p: 0,
              ...
              >
                ...
                <tbody style={{overflowX: 'scroll'}}>
                  { chargepoint?.connectors?.map((e, index) => {
                    const status: ConnectorStatus = e?.statusName?.toLowerCase() as ConnectorStatus || "unavailable"
